...
const Header = () => {
    ...
    const { headerIsHome, setheaderIsHome } = useContext(AppContext)

    return (
        <>
            <header className={`flex justify-center w-full absolute ${location.pathname === '/ ' || headerIsHome ? 'translate-y-7' : 'bg-white border-b'} z-[999]`}>
                <div className={`w-primary py-3 flex justify-between items-center ${location.pathname === '/' || headerIsHome ? 'p-14' : '!p-5'}`}>
                    <Link className={`font-semibold text-[23px] ${location.pathname === '/' || headerIsHome ? 'text-white' : ''}`}>
                    ...
                    <nav className={`absolute w-full left-0 lg:static lg:flex lg:!p-0 lg:w-fit ${menuOpen ? 'flex' : 'hidden'} ${
                        location.pathname === '/' || headerIsHome
                            ? 'px-14 top-[4.5rem]'
                            : 'px-5 top-[5.5rem]   '
                    }`}> 
                        <ul className={`gap-5 lg:flex flex-col p-5 bg-white border border-stone-200 lg:p-0 lg:flex-row lg:bg-transparent lg:border-none w-full border ${menuOpen ? 'flex' : 'hidden'} ${location.pathname === '/' || headerIsHome
                            ? 'rounded-md'
                            : 'rounded-b-md'
                        }`}>
                            {
                    ...
                    <div className="flex items-center justify-center gap-5">
                        ...
                        <div className="lg:hidden">
                            <Hamburger  
                                color={`${location.pathname === '/' || headerIsHome ? 'white' : 'black'}`}
                                onToggle={toggled => {
                                    toggled
                                        ? setmenuOpen(true)
                                        : setmenuOpen(false)
